...
def _single_bootstrap_to_disk(
    ...
    # Compute results
    bootstrap_result_sample_dict, downsampled_df_sample_dict = compute_gene_discovery_downsampled_stats(
        filtered_molecule_infos, target_total_reads, n_steps, min_reads
    )

    # Save downsampled_df_dict to disk
    with open(os.path.join(output_dir, f"bootstrap_{bootstrap_idx}_downsampled_mol_infos_at_different_depths.dill"), "wb") as f:
        dill.dump(downsampled_df_sample_dict, f)

    # Save lightweight summary stats
    with open(os.path.join(output_dir, f"bootstrap_{bootstrap_idx}_stats_at_different_depths.dill"), "wb") as f:
        dill.dump(bootstrap_result_sample_dict, f)

    return bootstrap_result_sample_dict


def run_bootstrap_downsampling_parallel_to_disk(
    filtered_molecule_infos,
    target_total_reads,
    output_dir,
    n_bootstraps=10,
    ...
    # Aggregate only the small summary statistics
    bootstrap_results_aggregated = defaultdict(lambda: defaultdict(list))
    for bootstrap_result_sample_dict in results:
        for sample_id, stats in bootstrap_result_sample_dict.items():
            for key, value in stats.items():
                bootstrap_results_aggregated[sample_id][key].append(value)

    # Compute summary stats (same as before)
    final_summary_stats = defaultdict(dict)
    for sample_id, stats in bootstrap_results_aggregated.items():
        for k, v in stats.items():
            arr = np.array(v)
            final_summary_stats[sample_id][f"{k}_mean"] = arr.mean(axis=0)
            final_summary_stats[sample_id][f"{k}_std"] = arr.std(axis=0)
...
def main():
    ...
    n_bootstraps = 2  # Set to 1 for testing, change to 10 for full run
    n_steps = 10
    min_reads = 10_000_000

    final_summary_stats = run_bootstrap_downsampling_parallel_to_disk(
    ...
    with open(os.path.join(OUTPUT_DIR, "final_summary_stats.dill"), "wb") as f:
        dill.dump(final_summary_stats, f)

    print("Downsampling and bootstrap analysis completed successfully.")
    pprint(final_summary_stats)
